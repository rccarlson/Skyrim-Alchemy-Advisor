...
namespace SkyrimPotionWindow
{
	public partial class MainWindow : Form
	{
		...
		EffectGrouping GetPotions(System.Threading.CancellationToken token)
		{
			...
			try
			{
				var collected_potions = new System.Collections.Concurrent.ConcurrentBag<Potion>();
				var result = Parallel.ForEach(EnumerateAllIngredientsCombinations(selectedIngredients), ((Ingredient a, Ingredient b, Ingredient c) input) =>
				{
					Potion potion = new Potion(input.a, input.b, input.c
						, alchemySkill
						, fortifyAlchemy
						, alchemistPerk
						, physicianPerk
					...
					if (potion.IsValid && potion.effects.Any(e => desiredEffectBox.CheckedItems.Contains(e.name)))
						collected_potions.Add(potion);
				});
				effectGrouping = new EffectGrouping(collected_potions);
			}
            catch (Exception ex)
            {
